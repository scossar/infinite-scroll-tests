...
export default function InfinitePrune() {
  ...
  const prunePages = (pages: PagedItems) => {
    let pruned;
    // set the min size super high for now. not really testing this here:
    if (Object.keys(pages).length > 50) {
  ...
  const itemList = Object.values(pagedItems).flat();
  ...
  const ItemsFlat = ({
    ...
    index: number;
    style: React.CSSProperties;
  }) => <div style={style}>{itemList[index].value}</div>;
